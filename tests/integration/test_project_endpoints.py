...
class TestProjectCRUD:
    ...
    async def test_list_projects_includes_own(self, client: AsyncClient, auth_headers, test_project):
        resp = await client.get("/api/v1/projects/", headers=auth_headers)
        assert resp.status_code == 200
        ids = [p["id"] for p in resp.json()]
        assert test_project["id"] in ids

    async def test_get_project_detail(self, client: AsyncClient, auth_headers, test_project):
        resp = await client.get(f"/api/v1/projects/{test_project['id']}", headers=auth_headers)
        assert resp.status_code == 200
        data = resp.json()
    ...
    async def test_cannot_access_other_user_project(self, client: AsyncClient, test_project):
...
class TestProjectMembers:
    async def test_owner_is_in_members_list(self, client: AsyncClient, auth_headers, test_project):
        resp = await client.get(
